...
export function RecentOrdersWidget() {
  const { data, isLoading } = trpc.buyers.orders.list.useQuery({
    page: 1,
    limit: 5,
    sort: '-createdAt',
  });
      ...
      <CardContent>
        {orders.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-xs text-gray-400 mt-1">Browse products to place your first order</p>
          </div>
        ) : (
          <div className="space-y-3">
                ...
                <div key={order.id} className="flex items-center justify-between p-3 border rounded-lg hover:bg-gray-50 transition-colors">
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-gray-500">
                      {supplierName} • {order.createdAt ? format(new Date(order.createdAt), 'MMM d, yyyy') : 'Unknown'}
                    </div>
                  </div>
