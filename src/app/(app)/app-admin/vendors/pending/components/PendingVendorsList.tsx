  ...
  if (!data || data.vendors.length === 0) {
    return (
      <div className="text-center py-12 bg-gray-50 rounded-lg">
        <p className="text-lg font-semibold text-gray-900">No pending suppliers</p>
    ...
    <div className="space-y-4">
              ...
              <TableRow key={vendor.id}>
                ...
                <TableCell className="text-right">
                  <VendorApprovalActions
                    vendorId={vendor.id}
                    vendorName={vendor.companyName || 'Supplier'}
                    onSuccess={() => {
                      refetch();
                      router.refresh();
                    }}
      ...
      {data.totalPages > 1 && (
        <div className="text-center text-sm text-gray-500">
          Showing {data.vendors.length} of {data.total} pending suppliers
        </div>
      )}
    </div>
  );
